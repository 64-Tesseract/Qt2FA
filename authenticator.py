...
def get_totp_token (secret, later = 0):
    """The TOTP token is just a HOTP token seeded with every 30 seconds."""
    return get_hotp_token(secret, intervals_no = int((time.time()) + later) // 30)
...
def main ():
    ...
    with open(os.path.join(rel, "secrets.json"), "r") as f:
        secrets = json.load(f)
    print(f"Seed: {int(time.time()) // 30}")
    print(f"Time: {int(time.time() % 30)} / 30")
    print("\nKeys:")
    for label, key in sorted(list(secrets.items())):
        print(f"    {label}:\t{get_totp_token(key)}")
